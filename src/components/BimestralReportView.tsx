...
interface BimestralReportViewProps {
  ...
  dataKeyFormat?: (turma: string, selectedBimestre: string) => string;
}

export default function BimestralReportView({
  gradesData,
  selectedBimestre,
  turmasList,
  selectedTurma,
  dataKeyFormat,
}: BimestralReportViewProps) {
  const getKey = (turma: string) => {
    if (dataKeyFormat) return dataKeyFormat(turma, selectedBimestre);
    return `${selectedBimestre}_${turma}_bimestral`;
  };
  ...
  const stats = useMemo(() => {
    ...
    turmas.forEach((turma) => {
      ...
      const key = getKey(turma);
      const data = gradesData[key] || [];

      const validGrades = data.filter((g) => typeof g.grade === "number") as {
        studentName: string;
        grade: number;
      }[];
      ...
      validGrades.forEach((g) => {
        if (highestGrade === null || g.grade > highestGrade)
          highestGrade = g.grade;
        if (lowestGrade === null || g.grade < lowestGrade)
          lowestGrade = g.grade;
      });
    ...
    const average =
      totalStudents > 0
        ? (allGrades.reduce((acc, g) => acc + g, 0) / totalStudents).toFixed(1)
        : "0.0";

    const chartData = selectedTurma
      ? (gradesData[getKey(selectedTurma)] || [])
          .filter((g) => typeof g.grade === "number")
          .map((g: any) => ({
            name: g.studentName,
            grade: g.grade,
            fill:
              g.grade > 7 ? "#10b981" : g.grade >= 5 ? "#f59e0b" : "#f43f5e",
          }))
      : turmas.map((t) => ({
          name: t,
          media:
            turmasStats[t].total > 0
              ? Number((turmasStats[t].sum / turmasStats[t].total).toFixed(1))
              : 0,
        }));

    let seriesChartData: any[] = [];
    let reprovadosList: { name: string; grade: number; turma?: string }[] = [];
    let missingList: { name: string; turma?: string }[] = [];
    ...
    if (!selectedTurma) {
      ...
      turmas.forEach((t) => {
        ...
        // Collect all reprovados and missing for the general report
        const data = gradesData[getKey(t)] || [];
        data.forEach((g: any) => {
          ...
          } else if (typeof g.grade !== "number" || g.grade === "") {
            missingList.push({ name: g.studentName, turma: t });
          }
        });
      });
      ...
      seriesChartData = Object.keys(seriesData).map((k) => ({
        ...
        Média:
          seriesData[k].total > 0
            ? Number((seriesData[k].sum / seriesData[k].total).toFixed(1))
            : 0,
      }));
    } else {
      // Collect reprovados for the specific turma
      const data = gradesData[getKey(selectedTurma)] || [];
      data.forEach((g: any) => {
        if (typeof g.grade === "number" && g.grade < 5) {
          reprovadosList.push({ name: g.studentName, grade: g.grade });
        } else if (typeof g.grade !== "number" || g.grade === "") {
          missingList.push({ name: g.studentName });
        }
      });
    }
    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center bg-white border border-slate-200 rounded-3xl min-h-[400px]">
        ...
        <p className="text-sm text-slate-500 max-w-sm">
          Insira notas na aba de Lançamento para visualizar o relatório.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6" id="bimestral-report-content">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Acompanhamento e estatísticas das turmas
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 lg:gap-4">
        <div className="bg-white p-3 lg:p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center gap-3">
          ...
          <div className="text-center sm:text-left">
            <p className="text-[11px] lg:text-sm font-medium text-slate-500 leading-tight">
              Alunos Avaliados
            </p>
          ...
          <div className="text-center sm:text-left flex flex-col">
            ...
            <p className="text-lg lg:text-xl font-bold text-slate-800 mt-0.5">
              <span className="text-emerald-600">
                {stats.highestGrade !== null ? stats.highestGrade : "-"}
              </span>{" "}
              /{" "}
              <span className="text-rose-600">
                {stats.lowestGrade !== null ? stats.lowestGrade : "-"}
              </span>
            </p>
          </div>
        </div>
        <div className="bg-white p-3 lg:p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center gap-3">
          <div className="w-10 h-10 lg:w-12 lg:h-12 bg-slate-100 text-slate-600 rounded-xl flex items-center justify-center shrink-0">
            <UserMinus size={20} className="lg:w-6 lg:h-6" />
          </div>
          <div className="text-center sm:text-left flex flex-col">
            <p className="text-[11px] lg:text-sm font-medium text-slate-500 leading-tight">
              S/ Nota ou Aus.
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col h-full">
          <h3 className="text-lg font-bold text-slate-800 mb-6">
            {selectedTurma
              ? `Notas dos Alunos - ${selectedTurma}`
              : "Média por Turma"}
          </h3>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col items-center">
          ...
          <div className="w-full flex justify-center gap-4 mt-2 text-xs">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-slate-600 font-medium">Excel.</span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-3 h-3 rounded-full bg-amber-500"></div>
              <span className="text-slate-600 font-medium">Regul.</span>
            </div>
            <div className="flex items-center gap-1.5">
      ...
      {/* Relatório de Séries */}
      {!selectedTurma &&
        stats.seriesChartData &&
        stats.seriesChartData.length > 0 && (
