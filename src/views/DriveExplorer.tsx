  ...
  const fetchFiles = async (token: string) => {
    ...
    try {
      ...
      const res = await fetch(
        `https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(q)}&fields=files(id,name,mimeType,starred,iconLink)&orderBy=folder,name`,
        {
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between shrink-0 gap-4">
        ...
        {accessToken ? (
          <button
            onClick={logout}
            className="bg-red-50 text-red-600 px-5 py-2.5 rounded-xl text-sm font-bold border border-red-100 hover:bg-red-100 transition-colors shadow-sm self-start sm:self-auto"
          >
            Desconectar
          </button>
        ) : (
      ...
      <div className="flex-1 bg-white border border-slate-200 rounded-3xl shadow-sm flex flex-col overflow-hidden relative">
        <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between gap-3">
          <div className="flex items-center text-sm font-bold text-slate-800">
            <span>
              {activeTab === "root" ? "Espaço de Trabalho" : "Favoritos"}
            </span>
          </div>
