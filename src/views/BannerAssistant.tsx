...
export default function BannerAssistant() {
  ...
  const initialMessage: Message = {
    ...
    role: "model",
    content:
      "Olá! Sou o **Assistente Banner**, seu parceiro para planejamento de aulas. Você pode enviar slides (imagens ou PDFs) aqui e eu vou analisá-los, resumi-los e sugerir atividades engajadoras que você pode aplicar em sala de aula. Como posso ajudar hoje?",
  };
    ...
    try {
      ...
      for (let file of currentFiles) {
        if (file.name.toLowerCase().endsWith(".pptx")) {
          ...
          parts.push({
            text: `[Conteúdo extraído do PPTX: ${file.name}]\n${text}`,
          });
        } else if (file.name.toLowerCase().endsWith(".ppt")) {
          parts.push({
            text: `[Aviso: O arquivo ${file.name} está no formato antigo (.ppt), que não é suportado para leitura automática. Se possível, salve como .pptx ou envie um PDF.]`,
          });
        } else {
      ...
      const errorMsg: Message = {
        ...
        role: "model",
        content:
          "Desculpe, ocorreu um erro ao processar sua mensagem ou analisar o arquivo. Tente novamente.",
      };
    ...
    <div className="flex h-[calc(100vh-8rem)] bg-slate-50 relative overflow-hidden rounded-2xl shadow-sm border border-slate-200">
      ...
      <div className="w-64 bg-white border-r border-slate-200 flex flex-col hidden md:flex shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-1">
          ...
          {sessions.length === 0 && (
            <div className="px-3 text-slate-400 text-sm text-center mt-4">
              Nenhum chat salvo.
            </div>
          )}
        </div>
        ...
        <div className="bg-white border-b border-slate-200 p-4 flex items-center justify-between shrink-0">
          ...
          <div className="hidden md:block">
            <p className="text-slate-500 text-sm font-medium">
              Analise slides, resuma conteúdos e planeje suas aulas.
            </p>
          </div>
        </div>
          ...
          <div className="max-w-5xl mx-auto space-y-6">
            {messages.map((message) => (
              ...
              >
                ...
                <div
                  className={`flex flex-col max-w-[85%] md:max-w-[75%] ${message.role === "user" ? "items-end" : "items-start"}`}
                >
                  {message.files && message.files.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-2 justify-end">
                      {message.files.map((file, i) => (
                        <div
                          key={i}
                          className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-lg border border-slate-200 shadow-sm text-xs text-slate-600"
                        >
          ...
          <div className="max-w-5xl mx-auto">
              ...
              {selectedFiles.length > 0 && (
                ...
                >
                  {selectedFiles.map((file, index) => (
                    <div
                      key={index}
                      className="flex items-center gap-2 bg-indigo-50 text-indigo-700 px-3 py-1.5 rounded-lg border border-indigo-100 text-sm"
                    >
                      <FileText size={14} />
            ...
            <div className="text-center mt-2 hidden md:block">
              <span className="text-[10px] md:text-xs text-slate-400 font-medium">
                O Assistente Banner pode analisar imagens, PDFs e apresentações
                PPTX para criar planejamentos de aula incríveis.
              </span>
            </div>
          </div>
        </div>
