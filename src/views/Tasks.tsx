...
export default function Tasks() {
  ...
  const addTask = (e: React.FormEvent) => {
    ...
    if (user) {
      setDoc(
        doc(db, "users", user.uid, "tasks", newTask.id.toString()),
        newTask,
  ...
  const removeTask = (id: number) => {
    ...
    if (user) {
      deleteDoc(doc(db, "users", user.uid, "tasks", id.toString())).catch((e) =>
        console.error(e),
      );
    }
  };
  ...
  const toggleTask = (id: number) => {
    setTasks(
      tasks.map((t) => {
        ...
        if (t.id === id && user) {
          setDoc(
            doc(db, "users", user.uid, "tasks", id.toString()),
            updated,
          ).catch((e) => console.error(e));
        }
        return updated;
  ...
  return (
    ...
    >
      <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
        ...
        <form
          onSubmit={addTask}
          className="flex flex-col md:flex-row gap-3 mb-8 bg-slate-50 p-4 rounded-2xl border border-slate-100"
        >
          <input
            type="text"
            value={input}
          ...
          <div className="flex gap-3">
            ...
            <button
              type="submit"
              className="bg-indigo-600 text-white rounded-xl px-6 font-bold hover:bg-indigo-700 transition-colors shadow-sm shrink-0 flex items-center justify-center"
            >
              <Plus size={20} />
            </button>
          </div>
            ...
            {tasks
              ...
              .map((task) => (
                ...
                >
                  ...
                  >
                    <button
                      className={`${task.done ? "text-emerald-500" : "text-slate-300 hover:text-indigo-500"} transition-colors shrink-0`}
                    >
                      {task.done ? (
                    ...
                    <div className="flex flex-col min-w-0">
                      <span
                        className={`text-lg font-medium truncate transition-all ${task.done ? "line-through text-emerald-600" : ""}`}
                      >
                        {task.text}
                      </span>
                      <div className="flex items-center gap-2 mt-1">
                        {task.priority && (
                          <span
                            className={`text-[10px] font-bold px-2 py-0.5 rounded-md border uppercase tracking-wider ${task.done ? "opacity-50" : priorityColors[task.priority]}`}
                          >
                        ...
                        {task.dueDate && (
                          <span
                            className={`flex items-center gap-1 text-[11px] font-bold ${task.done ? "text-emerald-600" : "text-slate-400"}`}
                          >
                            <Calendar size={12} />
                            {new Date(
