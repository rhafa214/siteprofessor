...
import { Camera, User, Users, Database, BookOpen, ChevronRight, Save, LogOut } from "lucide-react";
import { auth } from "../lib/firebase";

export default function ProfileView({ setCurrentView }: { setCurrentView: (v: any) => void }) {
  ...
  if (!user) {
     return (
        <div className="flex flex-col items-center justify-center p-12 h-full text-slate-500">
           ...
           <p className="text-xl font-bold text-slate-800">Faça login para ver seu perfil</p>
        </div>
     );
  }
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        >
          <LogOut size={18} /> <span className="hidden sm:inline">Sair da Conta</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="flex items-center gap-6">
             <div className="relative shrink-0">
               <div className="w-24 h-24 rounded-full border-4 border-indigo-50 shadow-md overflow-hidden bg-slate-100">
                 <img
                   src={photoURL || "https://ui-avatars.com/api/?name=Professor&background=6366f1&color=fff"}
                   alt="Perfil"
             ...
             <div className="flex flex-col flex-1 min-w-0 font-medium">
                {isEditing ? (
                  <div className="flex flex-col gap-3 w-full">
                     <label className="text-sm font-bold text-slate-600">Nome de Exibição</label>
                     <input 
                       type="text" 
                       value={displayName}
                     ...
                     />
                     <label className="text-sm font-bold text-slate-600 mt-2">URL da Nova Foto</label>
                     <input 
                       type="text" 
                       value={photoURL}
                  ...
                  <div className="flex flex-col items-start min-w-0">
                     <span className="text-xl font-bold text-slate-800 truncate block w-full">{user.displayName || "Sem Nome"}</span>
                     <span className="text-sm text-slate-500 truncate mb-4">{user.email}</span>
                     <button
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 lg:p-8 flex flex-col gap-6">
          <div className="mb-2">
            <h2 className="text-lg font-bold text-slate-800">Configurações e Banco de Dados</h2>
            <p className="text-sm font-medium text-slate-500 mt-1">Gerencie os dados sensíveis do sistema.</p>
          </div>
          ...
          <div className="flex flex-col gap-3">
             ...
             >
                <div className="flex items-center gap-3">
                   ...
                   <div className="flex flex-col text-left">
                      <span className="font-bold text-slate-800">Banco de Alunos</span>
                      <span className="text-xs font-medium text-slate-500">Gerenciar alunos, RAs e turmas</span>
                   </div>
                </div>
             ...
             >
                <div className="flex items-center gap-3">
                   ...
                   <div className="flex flex-col text-left">
                      <span className="font-bold text-slate-800">Documentos Jarvis</span>
                      <span className="text-xs font-medium text-slate-500">Base de conhecimento da IA</span>
                   </div>
                </div>
             ...
             >
                <div className="flex items-center gap-3">
                   ...
                   <div className="flex flex-col text-left">
                      <span className="font-bold text-slate-800">Treinamento & Datas</span>
                      <span className="text-xs font-medium text-slate-500">Dados do ano e feriados</span>
                   </div>
                </div>
             ...
             >
                <div className="flex items-center gap-3">
                   <div className="w-10 h-10 rounded-xl bg-blue-100 text-blue-600 flex items-center justify-center shrink-0">
                      <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="3" y1="12" x2="21" y2="12"></line><line x1="3" y1="6" x2="21" y2="6"></line><line x1="3" y1="18" x2="21" y2="18"></line><line x1="12" y1="3" x2="12" y2="21"></line></svg>
                   </div>
                   <div className="flex flex-col text-left">
                      <span className="font-bold text-slate-800">Grade de Horários</span>
                      <span className="text-xs font-medium text-slate-500">Cronograma de aulas da semana</span>
                   </div>
                </div>
