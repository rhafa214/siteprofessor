...
export default function ProvaPaulistaAnalysis({
        ...
        if (user) {
          ...
          if (snap.exists()) {
            ...
            if (oldSnap.exists()) {
              ...
              const localData =
                localStorage.getItem(`pp_${bKey}_${selectedTurma}`) ||
                localStorage.getItem(`pp_${selectedTurma}`);
              setClassData(
  ...
  const handleAddExam = () => {
    ...
    const newExams = [
      ...classData.exams,
      { ...newExam, id, date: new Date().toISOString() },
    ];
  ...
  const syncStudents = async () => {
    ...
    try {
      ...
      if (snap.exists() && snap.data().students) {
        ...
        studentsFromTasks.forEach((s: any) => {
          if (!newGrades[s.id]) newGrades[s.id] = {};
        });
        saveClassData({
          ...classData,
          students: studentsFromTasks,
          grades: newGrades,
        });
        showAlert("Alunos sincronizados com sucesso!", "Sucesso", "success");
      } else {
        showAlert(
          "Nenhum aluno encontrado no Banco de Alunos ou Controle de Tarefas para esta turma.",
          "Aviso",
          "warning",
        );
      }
  ...
  const updateGrade = (studentId: string, examId: string, value: string) => {
    ...
    if (exam && numVal !== null && numVal > exam.maxScore)
      numVal = exam.maxScore;
    if (numVal !== null && numVal < 0) numVal = 0;
    const newGrades = {
      ...classData.grades,
      [studentId]: { ...classData.grades[studentId], [examId]: numVal },
    };
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 font-medium">
            Acompanhe as notas da Prova Paulista e os descritores.
          </p>
        </div>
      </div>
            ...
            <div className="p-4 flex-1 overflow-auto">
              <BimestralReportView
                gradesData={turmasList.reduce((acc, t) => {
                  const bKey = selectedBimestre.replace("º Bimestre", "");
                  let localData = localStorage.getItem(`pp_${bKey}_${t}`);
                  if (!localData && bKey === "2") {
                    localData = localStorage.getItem(`pp_${t}`);
                  }
                  if (localData) {
                    try {
                      ...
                      acc[t] = parsedClass.students.map((s) => {
                         let totalScore = 0;
                         let totalMax = 0;
                         parsedClass.exams.forEach(ex => {
                           const g = parsedClass.grades[s.id]?.[ex.id];
                           if (g !== undefined && g !== null) {
                             totalScore += g;
                             totalMax += ex.maxScore;
                           }
                         });
                         const grade = totalMax > 0 ? (totalScore / totalMax) * 10 : 0;
                         return { id: s.id, studentName: s.name, grade: Number(grade.toFixed(1)) };
                      });
                    } catch (e) {}
                  }
                  return acc;
              ...
              <BimestralReportView
                gradesData={{
                  [selectedTurma]: classData.students.map((s) => {
                    ...
                    let totalMax = 0;
                    classData.exams.forEach((ex) => {
                      const g = classData.grades[s.id]?.[ex.id];
                      if (g !== undefined && g !== null) {
                        totalScore += g;
                        totalMax += ex.maxScore;
                      }
                    ...
                    const grade =
                      totalMax > 0 ? (totalScore / totalMax) * 10 : 0;
                    return {
                      id: s.id,
                      studentName: s.name,
                ...
                <div className="p-4 bg-indigo-50 border-b border-indigo-100 flex items-end gap-4">
                  <div className="flex-1 max-w-sm">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 rounded-lg border-none focus:ring-2 focus:ring-indigo-500 outline-none"
                    />
                  </div>
                  <div className="w-32">
                    <label className="block text-xs font-bold text-slate-500 mb-1">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl h-10"
                  >
                    Adicionar
                  </button>
                  <button
                    onClick={() => setIsAddingExam(false)}
                    className="px-4 py-2 text-slate-500 hover:text-slate-800 font-bold rounded-xl h-10"
                  >
                    Cancelar
                  </button>
                </div>
                ...
                {isLoading ? (
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-center max-w-md mx-auto">
                    ...
                    <p className="text-slate-500 mb-6">
                      Você precisa ter alunos cadastrados nesta turma no Banco
                      de Alunos ou Controle de Tarefas e Sincronizá-los.
                    </p>
                    <button
                      onClick={syncStudents}
                      className="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold"
                    >
                      Importar Alunos Sincronizados
                    </button>
                  </div>
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-center max-w-md mx-auto">
                    ...
                    <p className="text-slate-500 mb-6">
                      Adicione uma Prova Paulista para começar a registrar os
                      resultados dos alunos.
                    </p>
                    <button
                      onClick={() => setIsAddingExam(true)}
                      className="px-6 py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-bold"
                    >
                      Nova Prova Paulista
                    </button>
                  </div>
                  ...
                  <div className="min-w-max border border-slate-200 rounded-xl overflow-hidden bg-white shadow-sm">
                    ...
                    {classData.students.map((student, i) => (
                      <div
                        key={student.id}
                        className={`grid grid-cols-[300px_minmax(100px,1fr)] border-b border-slate-100 transition-colors hover:bg-indigo-50/30 ${i % 2 === 0 ? "bg-white" : "bg-slate-50/50"}`}
                      >
                        <div className="p-3 border-r border-slate-200 sticky left-0 bg-inherit z-10 flex items-center justify-between font-medium text-sm text-slate-700">
                          ...
                          <span className="font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-lg text-xs">
                            {calculateStudentAvg(student.id)}
                          </span>
                        </div>
                        <div className="flex">
                          {classData.exams.map((ex) => (
                            <div
                              key={ex.id}
                              className="min-w-[120px] p-2 border-r border-slate-200 flex-1 flex items-center justify-center"
                            >
                              <input
                                type="number"
                                min="0"
                                max={ex.maxScore}
                                value={
                                  classData.grades[student.id]?.[ex.id] ?? ""
                                }
                                onChange={(e) =>
                                  updateGrade(student.id, ex.id, e.target.value)
                                }
                                className="w-16 text-center text-sm font-bold bg-transparent border-none focus:ring-2 focus:ring-indigo-500 rounded-lg p-1 outline-none"
                                placeholder="--"
                              />
                            </div>
                          ))}
