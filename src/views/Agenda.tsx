  ...
  const getEventColor = (str: string) => {
    const colors = [
      ...
      },
      {
        ...
        textTime: "text-fuchsia-600",
        textTitle: "text-fuchsia-900",
      ...
      <div className="bg-indigo-600 text-white p-6 rounded-3xl shadow-md border border-indigo-500 relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-6 shrink-0">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          <div className="flex items-center gap-3 bg-indigo-700/50 p-3 rounded-2xl border border-indigo-500/50 flex-1 md:w-48">
            ...
            <div>
              <div className="text-[10px] text-indigo-200 font-bold uppercase tracking-wider mb-0.5">
                Pendentes
              </div>
              ...
              {user ? (
                <>
                  {isLoading ? (
                    ...
                    todayEvents.reduce((acc, ev, i) => {
                      const startInfo = ev.start?.dateTime
                        ? new Date(ev.start.dateTime)
                        : ev.start?.date
                          ? new Date(ev.start.date)
                          : new Date();
                      const endInfo = ev.end?.dateTime
                        ? new Date(ev.end.dateTime)
                        : ev.end?.date
                          ? new Date(ev.end.date)
                          : new Date();
                      const timeStr = ev.start?.dateTime
                        ? startInfo.toLocaleTimeString([], {
                            hour: "2-digit",
                            minute: "2-digit",
                          })
                        : "Dia todo";
                      ...
                      if (
                        isFuture &&
                        !acc.some((el: any) => el?.key === "agora-marker")
                      ) {
                        acc.push(
                          <div
                            key="agora-marker"
                            className="flex items-center gap-3 my-4 opacity-80"
                            id="agora-marker"
                          >
                            <div className="flex-1 border-t-2 border-red-400 border-dashed"></div>
                            <span className="text-xs font-bold text-red-500 bg-red-50 px-2 py-0.5 rounded-md border border-red-100">
                              AGORA
                            </span>
                            <div className="flex-1 border-t-2 border-red-400 border-dashed"></div>
                          </div>,
                      ...
                      acc.push(
                        <div
                          key={ev.id || i}
                          className={`flex gap-4 p-4 rounded-2xl border transition-all ${isCurrent ? `${c.activeBg} ${c.border} shadow-md scale-[1.01] ring-2 ring-opacity-50` : isPast ? "bg-slate-50 border-slate-100 opacity-60" : `${c.bg} ${c.border} shadow-sm`} hover:-translate-y-0.5 hover:shadow-md`}
                        >
                          <div
                            className={`font-mono font-bold text-sm shrink-0 pt-1 ${isPast ? "text-slate-400" : c.textTime}`}
                          >
                            {timeStr}
                          </div>
                          <div>
                            <h4
                              className={`font-bold text-sm ${isPast ? "text-slate-500 line-through decoration-slate-300" : c.textTitle}`}
                            >
                              {ev.summary || "Evento sem título"}
                            </h4>
                            <div
                              className={`flex items-center gap-3 mt-1.5 text-xs font-bold opacity-70 ${isPast ? "text-slate-400" : c.textTitle}`}
                            >
                      ...
                      if (
                        ...
                        acc.push(
                          <div
                            key="agora-marker"
                            className="flex items-center gap-3 mt-6 opacity-80"
                          >
                ...
                <div className="h-full flex flex-col items-center justify-center text-center p-4 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50/50">
                  ...
                  <span className="text-xs text-slate-400 max-w-[200px]">
                    Os eventos do seu dia aparecerão aqui.
                  </span>
                </div>
              )}
            </div>
        ...
        <div className="xl:w-2/3 flex-1 bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
          <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center shrink-0">
            <h2 className="font-bold text-slate-700 flex items-center gap-2">
              <CalendarIcon size={18} className="text-indigo-500" /> Visão
              Semanal
            </h2>
            <div className="flex items-center gap-3">
              <div className="flex items-center bg-white border border-slate-200 rounded-lg shadow-sm">
                ...
                <button
                  ...
                  className="px-3 py-1.5 text-xs font-bold text-slate-600 hover:text-indigo-600 border-x border-slate-200 transition-colors"
                >
                  {weekOffset === 0
                    ? "Esta Semana"
                    : weekOffset < 0
            ...
            {user ? (
                ...
                {["Segunda", "Terça", "Quarta", "Quinta", "Sexta"].map(
                  (day, dIdx) => {
                    ...
                    return (
                      <div
                        key={day}
                        className="flex-1 min-w-[200px] bg-white border border-slate-200 rounded-2xl flex flex-col snap-start overflow-hidden"
                      >
                        <div
                          className={`p-3 border-b text-center font-bold shrink-0 ${diff === 0 && weekOffset === 0 ? "bg-indigo-50 border-indigo-100 text-indigo-700" : "bg-slate-50 border-slate-100 text-slate-600"}`}
                        >
                          {day}{" "}
                          ...
                          {dayEvents.length > 0 ? (
                            dayEvents.map((ev, i) => {
                              ...
                              const endInfo = ev.end?.dateTime
                                ? new Date(ev.end.dateTime)
                                : null;
                              const timeStr = startInfo
                                ? `${startInfo.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })} - ${endInfo ? endInfo.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "?"}`
                                : "Dia todo";
                              ...
                              return (
                                <div
                                  key={ev.id || i}
                                  className={`${c.bg} border ${c.border} shadow-sm ${c.shadow} p-3 rounded-xl mb-2 transition-all hover:-translate-y-1 hover:shadow-md cursor-pointer`}
                                >
                                  <div
                                    className={`text-[10px] font-bold ${c.textTime} mb-1 uppercase tracking-wider`}
                                  >
                                    {timeStr}
                                  </div>
                                  <div
                                    className={`text-xs font-bold ${c.textTitle} leading-tight`}
                                  >
                                    {ev.summary || "Evento"}
                                  </div>
                                </div>
              ...
              <div className="absolute inset-0 m-4 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center text-center p-8 bg-white/60 backdrop-blur-sm">
                ...
                <h3 className="text-2xl font-bold text-slate-800 mb-3 tracking-tight">
                  Sincronize sua Rotina
                </h3>
                <p className="text-slate-500 max-w-md mx-auto mb-8 leading-relaxed font-medium">
                  Integre sua conta do Google para visualizar seu Google
                  Calendar nativamente aqui. Nós organizaremos seu dia e
                  destacaremos o que é mais importante.
                </p>
