...
export default function GradePlanConfigView({ selectedBimestre }: { selectedBimestre: string }) {
      ...
      try {
        ...
        if (activePlan) {
          ...
          const newComps: CanonicalGradeComponent[] = [
             ...
             { id: 'new_2', gradePlanId: '', key: 'BIMESTRAL', label: 'Avaliação', weight: 30, sourceType: 'MANUAL', sourceKey: 'BIMESTRAL', enabled: true, order: 2 },
             ...
             { id: 'new_5', gradePlanId: '', key: 'PARTICIPACAO', label: 'Participação', weight: 10, sourceType: 'MANUAL', sourceKey: 'PARTICIPACAO', enabled: true, order: 5 },
          ];
  ...
  const handleSave = async (activate: boolean) => {
    ...
    try {
      ...
      if (activate) {
        await gradePlanService.activateGradePlan(user.uid, newPlan, finalComps);
