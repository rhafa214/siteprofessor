...
export const DATAS_OFICIAIS = {
  provas: [
    {
      nome: "Prova Paulista - 1º Bim",
      data: "2026-04-13",
      dataFim: "2026-04-17",
    },
    {
      nome: "Prova Paulista - 2º Bim",
      data: "2026-06-15",
      dataFim: "2026-06-19",
    },
    {
      nome: "Prova Paulista - 3º Bim",
      data: "2026-09-21",
      dataFim: "2026-09-25",
    },
...
export function getHolidays(year: number): Record<string, string> {
  ...
  const sSanta = new Date(pascoa);
  sSanta.setDate(pascoa.getDate() - 2);
  const carnVal = new Date(pascoa);
  carnVal.setDate(pascoa.getDate() - 47);
  const corpVal = new Date(pascoa);
  corpVal.setDate(pascoa.getDate() + 60);
...
export function getSmartPhrase(): string {
  ...
  if (vindo)
    return dias === 1
      ? r([`Amanhã você tem um respiro! 🙌`, `Amanhã é folga! 🏖️`])
      : r([
          `Essa semana é mais curta! ✨`,
          `Faltam ${dias} dias para o feriado! 🚀`,
        ]);
